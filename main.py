...
from pandac.PandaModules import WindowProperties, AntialiasAttrib, OdeWorld, PStatClient, PNMImage, OdeSimpleSpace, OdeJointGroup, OdeTriMeshGeom, OdeTriMeshData, NodePath, Texture, TransparencyAttrib, OdePlaneGeom, AmbientLight, VBase4, DirectionalLight, OdeUtil, Vec3, Mat3
...
class Game(ShowBase):
    ...
    def onCollision(self, entry):
        ...
        for _player in self.players:
            ...
            if geom1.compareTo(self.plane) == 0 and player.vehicle.physics_model.compareTo(body2) == 0 or geom2.compareTo(self.plane) == 0 and _player.vehicle.physics_model.compareTo(body1) == 0:
                _player.vehicle.physics_model.setPosition(0, 0, 20)
                _player.vehicle.physics_model.setLinearVel(0, 0, 0)
                _player.vehicle.physics_model.setTorque(0, 0, 0)
                _player.vehicle.physics_model.setRotation(Mat3.rotateMat(0, (Vec3(0, 0, 1))))
                return

            # Decrease energy on collision
            elif _player.vehicle.physics_model.compareTo(body1) == 0 or _player.vehicle.physics_model.compareTo(body2) == 0:
                _player.vehicle.energy -= _player.vehicle.physics_model.getLinearVel().length() * 0.1
                _player.updateOSD()
    # -----------------------------------------------------------------
    ...
    def updatePos(self, task):
        ...
        positionen = []
        for _player in self.players:
            positionen.append(_player.position)
        positionen.sort()
        for _player in self.players:
            _player.rank = positionen.index(_player.position)
            # self._notify.debug( ("PlayerRank", _player.rank ))
        return task.again
    ...
    def gameTask(self, task):
        ...
        while self.deltaTimeAccumulator > self.stepSize:  # Step the simulation
            for _player in self.players:
                _player.doStep()  # refresh player specific things (rays)
                # get the player input and set the forces
                if _player.device.boost:
                    _player.vehicle.setBoost()
                else:
                    _player.vehicle.stopBlowout()

                if _player.device.directions[0] != 0 or _player.device.directions[1] != 0:
                    _player.vehicle.direction = _player.device.directions
                linear_velocity = _player.vehicle.physics_model.getLinearVel()
                angular_velocity = _player.vehicle.physics_model.getAngularVel()
                mass = _player.vehicle.physics_model.getMass().getMagnitude()

                # calculate airresistance to get energy out of the ode-system
                _player.vehicle.physics_model.addForce(linear_velocity * -self.LINEAR_FRICTION * mass)
                _player.vehicle.physics_model.addTorque(angular_velocity * -self.ANGULAR_FRICTION * mass)

                # calculate the ray
                col = OdeUtil.collide(_player.vehicle.ray.getRay(), self.groundGeom)
                if not col.isEmpty():
                    self.onRayCollision(col, _player)  # handles collisions from the ray with the street

                col = OdeUtil.collide(_player.vehicle.frontray.getRay(), self.groundGeom)
                if not col.isEmpty():
                    self.onFrontRayCollision(col, _player)

                # Collision with the border
                col = OdeUtil.collide(_player.vehicle.collision_model, self.borderr)
                if not col.isEmpty():
                    self.onBorderCollision(col, _player)
                else:
                    col = OdeUtil.collide(_player.vehicle.collision_model, self.borderl)
                    if not col.isEmpty():
                        self.onBorderCollision(col, _player)
        ...
        for _player in self.players:  # set new rank
